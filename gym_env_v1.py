...
class MTICLEnv(gym.Env):
    ...
    def __init__(self, src_lang, trg_lang, file_data, file_data_icl_examples, **kwargs):
        ...
        assert self.state_window_length > 0
        assert self.state_representation in (
            "model_single_representation",
            "sentence_and_actions",
            "model_single_representation+sentence_and_actions",
            "representation_per_token_with_features",
            "representation_last_token_current_and_relative_diff",
            "representation_mean_plus_last_75_perc_layer_and_relative_diff",
            "representation_mean_75_perc_layer",
            "representation_last_75_perc_layer",
        ...
        if self.state_representation == "model_single_representation" and self.state_window_length > 1:
            ...
            if not self.process_token_time_step:
                self.state_window_length -= 1
        elif self.state_representation in ("representation_mean_75_perc_layer", "representation_last_75_perc_layer"):
        ...
        if self.state_representation in ("representation_per_token_with_features", "representation_per_token_with_features_v2"):
            ...
            if self.embedding_pooling_model_layer not in (-1, "100%"):
            ...
            assert self.embedding_pooling_model_method_state in ("last", "mean"), self.embedding_pooling_model_method_state
        ...
        if self.state_representation in ("representation_mean_75_perc_layer", "representation_last_75_perc_layer"):
            self.state_dim = self.state_dim_per_token + self.state_dim_per_token_time_step + self.action_dim

            if not self.process_token_time_step:
                self.state_dim -= self.state_dim_per_token_time_step

        if self.state_representation in "representation_one_hot_representation_time_and_selected_icl_examples":
        ...
        if self.state_representation == "representation_per_token_with_features_v2":
            ...
            self.state_dim += self.state_dim_per_token * (self.state_window_length - 4) # -4 for the 4 features
        ...
        if self.apply_l2_normalization_state and self.state_representation in ("representation_per_token_with_features", "representation_one_hot_representation_time_and_selected_icl_examples_external_embedding_src_and_last_example", "representation_per_token_with_features_v2"):
    ...
    def _reset_state(self):
        ...
        if self.state_representation == "representation_one_hot_representation_time_and_selected_icl_examples_external_embedding_src_and_last_example":
            ...
            for idx in range(3, self.state_window_length - 1):
    ...
    def _soft_reset(self, seed=None, options=None):
        ...
        if self.state_representation == "representation_per_token_with_features":
        ...
        elif self.state_representation in ("representation_last_token_current_and_relative_diff", "representation_mean_plus_last_75_perc_layer_and_relative_diff", "representation_mean_75_perc_layer", "representation_last_75_perc_layer"):
            token_representations = self.get_state_representation([src_sentence])[0] # right-to-left (last tokens are first) and right-padded LLM representation
            ...
            if self.state_representation not in ("representation_mean_75_perc_layer", "representation_last_75_perc_layer"):
            ...
            sum_zero = is_zero_vector.sum(axis=0).item()
            used_tokens = token_representations.shape[0] - sum_zero
            num_tokens = min(used_tokens, self.state_window_length - 4)
            ...
            self.logger_wrapper(gym.logger.debug, "representation_per_token_with_features_v2: %s tokens (used: min(%d, %d))", token_representations.shape, used_tokens, self.state_window_length - 4)
            self.logger_wrapper(gym.logger.debug, "Time step representations: %s", self.current_state_window[2])
    ...
    def apply_step(self, current_action):
        ...
        if self.state_representation == "model_single_representation":
            ...
            self.current_state_window[4][icl_idx] += 1.0 # set one-hot representation of selected ICL example
            ...
            assert np.isclose(sum(self.current_state_window[4]), len(self.current_icl_examples)), f"{self.current_state_window[4]} vs {len(self.current_icl_examples)}"
            ...
            for idx in range(num_tokens):
                self.current_state_window[idx + 3] = observation[idx]

                assert idx + 1 < len(self.current_state_window), "this should not happen: last position is reserved for the history of selected actions"
            ...
            self.current_state_window[-1][icl_idx] += 1e-4 * self.time_step # add small value to break ties between examples selected the same number of times, so the order of selection also matters
        else:
        ...
        if self.penalty_duplicate_example and not self.is_eval_env:
            assert self.eval_strategy_training != "actions-bm25"

            all_icl_examples_str = list(map(lambda i: '\t'.join(i), self.current_icl_examples))
            last_icl_example_str = '\t'.join(self.current_icl_examples[0 if self.current_icl_examples_prepend else -1])

            assert all_icl_examples_str.count(last_icl_example_str) > 0, last_icl_example_str

            add_penalty = all_icl_examples_str.count(last_icl_example_str) > 1

            if add_penalty:
                assert False, "This should not happen due to action masking" # TODO remove?

                if self.eval_strategy_training == "target_sentence_neg_ppl_reward":
                    # [-inf, 0]
                    penalty = 10
                else:
                    # [0, 100]
                ...
                self.logger_wrapper(gym.logger.info, "Penalty (=%s) for duplicate example applied: %s -> %s", penalty, reward, reward - penalty)
    ...
    def get_score_from_icl_example_bm25(self, remove_overlapping_actions=True):
        ...
        assert all(len(current_icl_example[-1]) == 2 for current_icl_example in self.current_icl_examples), len(self.current_icl_examples[-1])
        assert all(isinstance(current_icl_example[0], str) for current_icl_example in self.current_icl_examples), type(self.current_icl_examples[-1][0])
    ...
    def get_scores_bm25(self, remove_overlapping_actions=False, remove_source_sentence_from_scores=True):
        ...
        tokenized_query = src_translation_candidate.split()
        scores = self.data_icl_examples_bm25.get_scores(tokenized_query)
        #top_n = self.data_icl_examples_bm25.get_top_n(tokenized_query, corpus, n=len(scores)) # debug
        ...
        if remove_overlapping_actions:
            ...
            for idx, current_icl_example in enumerate(self.current_icl_examples):
                current_icl_example_str = str(current_icl_example[0])

                if current_icl_example_str in self.data_icl_examples_bm25_corpus:
                    idx2 = self.data_icl_examples_bm25_corpus.index(current_icl_example_str, found_idx)
